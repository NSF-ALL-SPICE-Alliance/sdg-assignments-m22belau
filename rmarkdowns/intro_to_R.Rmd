...
my_name <- "Berylin Lau" # run the name of your variable to see the output
```
...
```{r}
sdr_data %>% 
  select(goal_7_score, goal_9_score)
```
...
![Source: https://datavizblog.files.wordpress.com/2013/04/andrew-abela-chart-chooser-in-color.jpg](https://datavizblog.files.wordpress.com/2013/04/andrew-abela-chart-chooser-in-color.jpg) There's an awesome resource called [The R Graph Gallery](https://r-graph-gallery.com/index.html) that helps you through that process! It's a great place to look for visualization inspiration and includes R template code for you to follow.
