...
```{r, warning = FALSE}
p <- ggplot(data = sdr_data, aes(x = goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  geom_point() +
  theme_minimal() +
  scale_color_viridis_d() +
  labs(
    title = "Goal 3 vs Goal 4 Scores",
    x = "Goal 3 Score",
    y = "Goal 4 Score",
    color = "Regions"
  )
...
leaflet(sdr_data_world_joined) %>% 
  ...
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, 
              color = ~colorQuantile("YlGnBu", goal_11_score)(goal_11_score), 
              label = mytext)
```
