...
```{r}
ggplot(data = sdr_data, aes(x= goal_9_score, y = goal_11_score, color = goal_8_score)) +
  geom_point()
```
The plot shows a positive correlation between Goal 9 and Goal 11 scores. Countries with higher Goal 8 scores cluster in the upper right, indicating that strong performance in industry/innovation tends to overlap with better outcomes for sustainable cities and decent work.
...
```{r}
ggplot(sdr_data, aes(x = goal_11_score, fill=regions_used_for_the_sdr)) + 
  geom_histogram(bins=33, color="black")
```
The plot shows most countries score between 50-90 on Goal 11, meaning most countries are making moderate to good progress on Sustainable Cities and Communities (Goal 11), but there is still significant room for improvement since few countries are achieving scores above 90. The countries that are purple (OECD countries) have higher score ranges, while pink (Sub-Saharan Africa) represents lower scores, which indicates less development in sustainable cities and communities.
