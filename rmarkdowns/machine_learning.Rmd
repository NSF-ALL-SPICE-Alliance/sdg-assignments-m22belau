...
```{r}
rf_u5mort <- randomForest(normalized_score_sdg3_u5mort ~ .,
                             data = sdr_data_imputed,
                             importance = TRUE)
...
ggplot(importance_df_top_10, aes(x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) +
  geom_bar(stat = "identity", fill = "steelblue", color = "black") +
  theme_minimal() +
  labs(title = "Most Important Variables in Predicting Under-5 Mortality",
       subtitle = "Top 10",
       y = "SDG Indicator",
       x = "Feature Importance (% Increase in Mean Squared Error)")

pdp::partial(rf_u5mort, pred.var = "normalized_score_sdg6_sanita", plot = TRUE)

pd <- pdp::partial(rf_u5mort, pred.var = c("normalized_score_sdg6_sanita", "normalized_score_sdg3_u5mort"))
plotPartial(pd)
```
